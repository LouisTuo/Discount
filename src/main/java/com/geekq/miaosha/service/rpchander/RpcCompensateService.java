...
public class RpcCompensateService {
  ...
  public ResultGeekQ<String> recharge() {
    ...
    /**
     * 拦截 校验状态 -- init 或 ROLLING_BACK则 返回
     *
     * <p>成功则返回已处理状态
     */
    ...
    /** 创建订单步骤 可定义一个VO 一个planorder 对应多个planstep 创建 PlanOrder 创建 planStep createOrderStep(vo); */
    ...
    //        //第一步请求民生
    //        steps.add(planStepLogic.buildStep(planOrder.getId(), PlanStepType.X_RECHARGE_CMBC,
    // PlanStepStatus.INIT));
    //        if (inputVo.getCouponId() != null) {
    //            //第二步使用优惠券
    //            steps.add(planStepLogic.buildStep(planOrder.getId(),
    // PlanStepType.X_RECHARGE_USE_COUPON, PlanStepStatus.INIT));
    //        }
    //        //第三步减扣主账户
    //        steps.add(planStepLogic.buildStep(planOrder.getId(), PlanStepType.X_RECHARGE_POINT,
    // PlanStepStatus.INIT));
    //        //第四部减扣子账户
    //        steps.add(planStepLogic.buildStep(planOrder.getId(),
    // PlanStepType.X_RECHARGE_SUB_POINT, PlanStepStatus.INIT));
    //        //第五步发送通知
    //        steps.add(planStepLogic.buildStep(planOrder.getId(), PlanStepType.X_RECHARGE_NOTIFY,
    // PlanStepStatus.INIT));
    //
    //        planStepDao.batchInsert(steps);

    /**
     * 调用Rpc接口 第几步错误则回滚前几步 并更新step状态
     *
     * <p>然后定时任务去处理 状态为INIT与ROLLBACK的 状态订单
     */
    //        HandlerParam handlerParam = new HandlerParam();
    //        handlerParam.setPlanOrder(planOrder);
    //        AutoInvestPlanRechargeOrderInputVo inputVo =
    // JsonUtil.jsonToBean(planOrder.getParams(), AutoInvestPlanRechargeOrderInputVo.class);
